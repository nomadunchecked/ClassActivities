...
let array = [];
data.forEach((arr) => array.push(arr.address.geo));
console.log(array);
...
data.forEach((item) =>
  newArr.push(
    `catchPhrase is ${item.company.catchPhrase} and company bs is ${item.company.bs}`
  )
);
console.log(newArr);
...
//   .forEach((item) => (item.innerHTML = "Anything"));
